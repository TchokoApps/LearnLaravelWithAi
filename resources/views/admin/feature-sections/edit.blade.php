        ...
        <div class="col-12">
            <div class="card">
                <div class="card-header pb-3">
                    <h6 class="mb-0">Edit Smart Financial Section</h6>
                </div>
                <div class="card-body">
                    <form action="{{ route('admin.feature-sections.update', $featureSection->id) }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('section_title') is-invalid @enderror"
                                   id="section_title" name="section_title" value="{{ old('section_title', $featureSection->section_title) }}" required>
                            @error('section_title')
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" class="form-control @error('feature_title') is-invalid @enderror"
                                   id="feature_title" name="feature_title" value="{{ old('feature_title', $featureSection->feature_title) }}" required>
                            @error('feature_title')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control @error('feature_description') is-invalid @enderror"
                                      id="feature_description" name="feature_description" rows="4" required>{{ old('feature_description', $featureSection->feature_description) }}</textarea>
                        ...
                        <div class="mb-3">
                            ...
                            @if($featureSection->feature_icon)
                                <div class="mb-2">
                                    <img src="{{ asset('storage/' . $featureSection->feature_icon) }}" alt="Current Icon" style="max-height: 100px;">
                                </div>
                            @endif
                            <input type="file" class="form-control @error('feature_icon') is-invalid @enderror"
                                   id="feature_icon" name="feature_icon" accept="image/*">
                            <small class="text-muted">Accepted formats: JPEG, PNG, GIF, SVG. Max size: 2MB. Leave empty to keep current icon.</small>
                            @error('feature_icon')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="number" class="form-control @error('display_order') is-invalid @enderror"
                                   id="display_order" name="display_order" value="{{ old('display_order', $featureSection->display_order) }}" min="0" required>
                            @error('display_order')
                                <div class="invalid-feedback d-block">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="mb-3">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="is_active" name="is_active"
                                       value="1" {{ old('is_active', $featureSection->is_active) ? 'checked' : '' }}>
                        ...
                        <div class="d-flex gap-2">
                            <button type="submit" class="btn btn-primary">
                                <i class="ni ni-check-bold"></i> Update Feature
                            </button>
